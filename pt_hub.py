...
def _get_master_key() -> str:
    ...
    key_path = os.path.join(os.getcwd(), MASTER_KEY_FILE)
...
def _load_encrypted_api_keys() -> dict:
    ...
    keys_file = os.path.join(os.getcwd(), API_KEYS_FILE)
...
def _save_encrypted_api_keys(keys: dict) -> None:
    ...
    try:
        ...
        # Make the encrypted file readable only by owner
        os.chmod(keys_file, 0o600)
    except Exception as e:
        raise Exception(f"Failed to save encrypted API keys: {e}")
...
def _set_api_key(provider: str, key_type: str, value: str) -> None:
    """Set a specific API key for a provider."""
    keys = _load_encrypted_api_keys()
    if provider not in keys:
        keys[provider] = {}
    keys[provider][key_type] = value
    _save_encrypted_api_keys(keys)
...
class PowerTraderHub(tk.Tk):
    def __init__(self):
        ...
        # On startup, create missing alt folders (no trainer copy needed)
        self._ensure_alt_coin_folders_on_startup()
    ...
    def _ensure_alt_coin_folders_on_startup(self) -> None:
        """
        Ensure all alt coin folders exist (no trainer copy needed).
        """
        for coin in self.coins:
            if coin == "BTC":
                continue
            folder = self.coin_folders.get(coin, None)
    ...
    def _build_menu(self) -> None:
        ...
        m_settings.add_command(label="General Settings...", command=self.open_settings_dialog)
        m_settings.add_command(label="Platform Settings...", command=self.open_platform_settings_dialog)
    ...
    def _clamp_panedwindow_sashes(self, pw: ttk.Panedwindow) -> None:
        ...
        # Start the training queue checker
        self.after(1000, self._check_training_queue)
    ...
    def start_neural(self) -> None:
        ...
        try:
            os.environ["USE_KUCOIN_API"] = "1" if bool(self.settings.get("enabled_platforms", {}).get("kucoin", True)) else "0"
        except Exception:
            pass
    ...
    def train_all_coins(self) -> None:
        ...
        if running_count >= max_concurrent:
            messagebox.showinfo(
                "Training queue full",
                f"Already training {running_count} coins. Wait for some to finish or increase 'max_concurrent_trainings' in settings."
            )
            return

        # Queue coins that need training
        coins_to_train = []
        for c in self.coins:
            # Check if already training
            if c in self.trainers and self.trainers[c].info.proc and self.trainers[c].info.proc.poll() is None:
                continue  # Already training
            coins_to_train.append(c)

        if not coins_to_train:
            messagebox.showinfo("All coins training", "All coins are already being trained.")
            return

        # Start training up to the limit
        started = 0
        for coin in coins_to_train:
            if running_count + started >= max_concurrent:
                break
            self.trainer_coin_var.set(coin)
            self.start_trainer_for_selected_coin()
            started += 1

        remaining = len(coins_to_train) - started
        if remaining > 0:
    ...
    def start_trainer_for_selected_coin(self) -> None:
        ...
        # All coins use the main trainer file for speed and maintainability
        ...
        if not os.path.isfile(trainer_path):
            messagebox.showerror(
                "Missing trainer",
                f"Cannot find trainer script at:\n{trainer_path}"
            )
            return
        ...
        try:
            ...
            try:
                running = [c for c in self._running_trainers()]
    ...
    def _check_training_queue(self) -> None:
        """Check if any trainings have completed and start queued ones."""
        try:
            ...
            # If we have capacity, look for coins that need training
            if running_count < max_concurrent:
                for coin in self.coins:
                    ...
                    # Check if this coin needs training (not recently trained)
                    if self._coin_needs_training(coin):
                        self.trainer_coin_var.set(coin)
                        self.start_trainer_for_selected_coin()
                        running_count += 1
            ...
            # Update status
            if running:
                self.trainer_status_lbl.config(text=f"running: {', '.join(running)}")
            else:
                self.trainer_status_lbl.config(text="(no trainers running)")

        except Exception as e:
            # Don't spam logs with queue check errors
            pass

        # Schedule next check
        try:
            self.after(5000, self._check_training_queue)  # Check every 5 seconds
        except Exception:
            pass


    def _coin_needs_training(self, coin: str) -> bool:
        """Check if a coin needs training (not recently trained)."""
        coin = coin.upper().strip()
        folder = self.coin_folders.get(coin, "")
        if not folder or not os.path.isdir(folder):
            return True  # Folder doesn't exist, needs training
        ...
        try:
            ...
            with open(stamp_path, "r", encoding="utf-8") as f:
                raw = (f.read() or "").strip()
            ts = float(raw) if raw else 0.0
            # Consider trained if trained within last 14 days
            return (time.time() - ts) > (14 * 24 * 60 * 60)
        except Exception:
            return True
    ...
    def open_settings_dialog(self) -> None:
        ...
        ttk.Label(frm, text="File and Folder Settings", foreground=DARK_FG, font=("", 12, "bold")).grid(row=r, column=0, sticky="w", padx=(0, 10), pady=(10, 6))
        r += 1
        ...
        ttk.Label(frm, text="Script Configuration", foreground=DARK_FG, font=("", 12, "bold")).grid(row=r, column=0, sticky="w", padx=(0, 10), pady=(10, 6))
        r += 1
        ...
        ttk.Label(frm, text="UI and Display Settings", foreground=DARK_FG, font=("", 12, "bold")).grid(row=r, column=0, sticky="w", padx=(0, 10), pady=(10, 6))
        r += 1
    ...
    def open_platform_settings_dialog(self) -> None:
        ...
        enabled_platforms = self.settings.get("enabled_platforms", DEFAULT_SETTINGS["enabled_platforms"])
        platform_vars = {}
        for platform in ["kucoin", "binance", "binance_us", "coinbase", "coingecko", "robinhood"]:
            platform_vars[platform] = tk.BooleanVar(value=bool(enabled_platforms.get(platform, False)))

        r = 0
        # Platform Settings
        ttk.Label(frm, text="Platform Settings", foreground=DARK_FG, font=("", 12, "bold")).grid(row=r, column=0, sticky="w", padx=(0, 10), pady=(10, 6))
        r += 1
        
        # Platform checkboxes
        platform_frame = ttk.Frame(frm)
        platform_frame.grid(row=r, column=0, columnspan=3, sticky="ew", pady=(0, 10))
        ...
        ttk.Label(frm, text="Setup Wizards:", foreground=DARK_FG, font=("", 12, "bold")).grid(row=r, column=0, sticky="w", padx=(0, 10), pady=(10, 6))
        r += 1
        
        wizard_frame = ttk.Frame(frm)
        wizard_frame.grid(row=r, column=0, columnspan=3, sticky="ew", pady=(0, 10))
        
        def create_setup_wizard(platform: str):
            """Create setup wizard for a platform."""
            if platform == "kucoin":
                self._open_kucoin_setup_wizard()
            elif platform == "binance":
                self._open_binance_setup_wizard()
            elif platform == "binance_us":
                self._open_binance_us_setup_wizard()
            elif platform == "coinbase":
                self._open_coinbase_setup_wizard()
            elif platform == "coingecko":
        ...
        wc = 0
        for platform in ["kucoin", "binance", "binance_us", "coinbase", "coingecko", "robinhood"]:
            btn = ttk.Button(wizard_frame, text=f"Setup {platform.title()}", 
                           command=lambda p=platform: create_setup_wizard(p))
            btn.grid(row=wr, column=wc, sticky="ew", padx=(0, 10), pady=2)
            wc += 1
            if wc >= 3:
                wc = 0
                wr += 1
        ...
        # API Keys management
        ttk.Label(frm, text="API Keys Management:", foreground=DARK_FG, font=("", 12, "bold")).grid(row=r, column=0, sticky="w", padx=(0, 10), pady=(10, 6))
        r += 1
        
        keys_frame = ttk.Frame(frm)
        keys_frame.grid(row=r, column=0, columnspan=3, sticky="ew", pady=(0, 10))
        
        def open_keys_folder():
            """Open the folder containing encrypted API keys."""
            try:
        ...
        def clear_all_keys():
            """Clear all encrypted API keys with confirmation."""
            if not messagebox.askyesno(
                "Delete All API Keys?",
                "This will delete all encrypted API keys for all platforms.\n\n"
                "You will need to run setup wizards again for any platforms you want to use.\n\n"
                "Are you sure?"
            ):
                return
            ...
            try:
                keys_file = os.path.join(os.getcwd(), API_KEYS_FILE)
                master_file = os.path.join(os.getcwd(), MASTER_KEY_FILE)
        ...
        def save():
            try:
                # Save platform enable/disable settings
                self.settings["enabled_platforms"] = {
                    platform: bool(platform_vars[platform].get())
                    for platform in ["kucoin", "binance", "binance_us", "coinbase", "coingecko", "robinhood"]
                }
                ...
                try:
                    ...
                    pass
                try:
                    messagebox.showinfo("Saved", "Platform settings saved.", parent=win)
                except Exception:
                    try:
                ...
                try:
                    messagebox.showerror("Error", f"Failed to save platform settings:\n{e}", parent=win)
                except Exception:
                    messagebox.showerror("Error", f"Failed to save platform settings:\n{e}")

    # ---- Platform Setup Wizards ----
    
    def _open_kucoin_setup_wizard(self) -> None:
        """Setup wizard for KuCoin API keys."""
        self._open_generic_api_wizard("kucoin", ["api_key", "api_secret", "passphrase"], 
                                    "KuCoin API Setup", "Enter your KuCoin API credentials:")

    def _open_binance_setup_wizard(self) -> None:
        """Setup wizard for Binance API keys."""
        self._open_generic_api_wizard("binance", ["api_key", "api_secret"], 
                                    "Binance API Setup", "Enter your Binance API credentials:")

    def _open_binance_us_setup_wizard(self) -> None:
        """Setup wizard for Binance US API keys."""
        self._open_generic_api_wizard("binance_us", ["api_key", "api_secret"], 
                                    "Binance US API Setup", "Enter your Binance US API credentials:")

    def _open_coinbase_setup_wizard(self) -> None:
        """Setup wizard for Coinbase API keys."""
        self._open_generic_api_wizard("coinbase", ["api_key", "api_secret"], 
                                    "Coinbase API Setup", "Enter your Coinbase API credentials:")

    def _open_coingecko_setup_wizard(self) -> None:
        """Setup wizard for CoinGecko (no API keys needed)."""
        messagebox.showinfo("CoinGecko Setup", "CoinGecko is a free API that doesn't require API keys.\n\nNo setup needed!")

    def _open_robinhood_setup_wizard(self) -> None:
        """Setup wizard for Robinhood API keys."""
        self._open_generic_api_wizard("robinhood", ["api_key", "private_key"], 
                                    "Robinhood API Setup", "Enter your Robinhood API credentials:")

    def _open_generic_api_wizard(self, platform: str, fields: List[str], title: str, description: str) -> None:
        """Generic API setup wizard for platforms."""
        wiz = tk.Toplevel(self)
        wiz.title(title)
        wiz.geometry("500x300")
        wiz.minsize(400, 250)

        frm = ttk.Frame(wiz, padding=12)
        frm.pack(fill="both", expand=True)

        ttk.Label(frm, text=description, justify="left").grid(row=0, column=0, columnspan=2, sticky="w", pady=(0, 12))

        field_vars = {}
        row = 1
        for field in fields:
            field_name = field.replace("_", " ").title()
            ttk.Label(frm, text=f"{field_name}:").grid(row=row, column=0, sticky="w", pady=6)

            var = tk.StringVar()
            # Load existing value
            existing = _get_api_key(platform, field)
            var.set(existing)
            
            if "secret" in field.lower() or "private" in field.lower() or "passphrase" in field.lower():
        ...
        def save_keys():
            try:
                for field, var in field_vars.items():
                    _set_api_key(platform, field, var.get().strip())
                messagebox.showinfo("Saved", f"{title} credentials saved securely.", parent=wiz)
                wiz.destroy()
            except Exception as e:
                messagebox.showerror("Error", f"Failed to save credentials: {e}", parent=wiz)

        ttk.Button(btns, text="Save", command=save_keys).pack(side="left")
